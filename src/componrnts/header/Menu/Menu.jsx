...
const Menu = (props) => {
	const creatMenuItems = (menuItems) => menuItems.map((el, i) => <li className="menu__item" onClick={props.onClickItem} key={i}>
		<NavLink to={'/login'}>{el}</NavLink></li>);
	const menuElements = props.isBurgerMenu ?
		(props.menuItems.map((elem, i) =>
			<ul className='menu__sublist' key={i}>{creatMenuItems(elem)}</ul>)
		) : creatMenuItems(props.menuItems)
	return (
